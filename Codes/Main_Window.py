...
from Codes.PathFindingTkinter import *
...
class Window:
    def __init__(self, root):
        ...
        # av=PhotoImage(file="./Images/algorithm_visualizer.png")
        # av=PhotoImage(file="E:/5th sem/SE/proj/Python Application/Images/algorithm_visualizer.png")
        # default
        # self.MainLabel = Label(self.root, text='Algorithm Visualizer', bg="LightSkyBlue1", fg="blue4",    font=("calibri italic", 20))
        # c1
        # self.MainLabel = Label(self.root, text='Algorithm Visualizer', bg="#1f2833", fg="#66fcf1",font=("Gill Sans Ultra Bold", 20))
        self.MainLabel = Label(self.root, text='Algorithm Visualizer', bg="#1f2833", fg="#66fcf1",font=("Gill Sans Ultra Bold", 20))
      
        # self.MainLabel = Label(self.root,image=av,height=200,width=200 )
        self.MainLabel.pack(pady=7)
        # varun_label = Label(image=av)
        # varun_label.place(x=100,y=100)

        self.MainLabel = Label(self.root, text='Visualize Algorithm For Better Understanding', bg="#1f2833", fg="white",font=("Gill Sans", 12))
        self.MainLabel.pack(pady=0)
        
       

        # Dictionary On types of Algorithms and their lists
        self.Algo = {'Searching': ['Linear Search', 'Binary Search'],
                     'Sorting': ['Selection Sort', 'Insertion Sort', 'Bubble Sort','Comb Sort','Cocktail Shaker Sort','Gnome Sort', 'Merge Sort', 'Quick Sort',
                                 'Heap Sort', 'Shell Sort', 'Radix Sort'],
                     'Backtracking': ['Sudoku', 'N-Queens', "Knight's Tour"],
                     'Path-Finding': ['BFS', 'DFS','A_star','Dijkstra']
                    }
        ...
        self.AlgoTypeList.config(bg="#c5c6c7", activebackground="white", cursor="hand2")
        self.AlgoNameList = OptionMenu(self.root, self.AlgoNameVar, 'None')
        self.AlgoNameList.config(bg="#c5c6c7", activebackground="white", cursor="hand2")
        # label of the two dropdown menus
        self.AlgoTypeVar.set("Select Algorithm Type")
        self.AlgoNameVar.set("Select Algorithm")
        self.AlgoTypeList.pack(pady=20)
        self.AlgoNameList.pack(pady=5)

        # next button
        # av=PhotoImage(file="Images/button.png")
        self.NextButton = Button(self.root, text="Next >",height=2,width=8, bg="#98FFA0", activebackground="#6EFF79",command=self.Run1)

        # self.NextButton = Button(self.root,image=av,command=self.Run1)
    ...
    def Run2(self):
        # This is for when the algorithm is not slected yet
        # if self.AlgoNameVar.get() == "Select Algorithm":
        ...
        elif self.AlgoTypeVar.get() == "Path-Finding":
            if self.AlgoNameVar.get() == "BFS":
                StartAlg()
            elif self.AlgoNameVar.get() == "DFS":
                StartAlg()
            elif self.AlgoNameVar.get() == "A_star":
                StartAlg()
            elif self.AlgoNameVar.get() == "Dijkstra":
                StartAlg()
    ...
    def Run1(self):
        ...
        elif self.AlgoTypeVar.get() != "Select Algorithm Type" and self.AlgoNameVar.get() == "Select Algorithm":
            messagebox.showerror("Error!", "Please select Algorithm.")
