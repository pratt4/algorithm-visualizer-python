...
def StartAlg():
    ...
    UI_frame = Frame(root, width=600, height=500, bg='black')
    ...
    def break_wall(current, new):
        if current.row == new.row:
            if current.col > new.col:
                ...
                wall = grid_array[current.row][current.col - 1]
            else:
                # wall to the right
                wall = grid_array[current.row][current.col + 1]
        else:
            if current.row > new.row:
                # wall above
                wall = grid_array[current.row - 1][current.col]
            else:
                # wall below
                wall = grid_array[current.row + 1][current.col]
        # break wall
    ...
    def StartAlgorithm():
        ...
        for row in grid_array:
            for spot in row:
                ...
                spot.f = float('inf')
                spot.parent = None
                spot.update_neighbors(grid_array)
        ...
        if algMenu.get() == 'A-star Algorithm':
            a_star(grid_array, speedScale.get())
        elif algMenu.get() == 'Breadth-First Algorithm':
            breadth_first(grid_array, speedScale.get())     
    ...
    def build_maze():
        ...
        if bldMenu.get() == 'Random walls':
            random_walls(grid_array)
        elif bldMenu.get() == 'Circular maze':
            circ_maze(grid_array, ROWS) 
        elif bldMenu.get() == 'Carved out maze':
            carve_out(grid_array, ROWS, speedScale.get())
    ...
    Button(UI_frame, text='Go Back', command=Back, font = ("Helvetica", 9),
    bg='white').grid(row=8, column=0, padx=5, pady=(10, 10))
    
    # Button(, bg="burlywood1", fg="RoyalBlue4", text="< Go Back to main menu", command=self.Back, cursor="hand2")
    # self.BackButton.grid_propagate(0)
    # self.BackButton.place(relx=0.2, rely=0.94)

    # Create grid
    grid_array = make_grid(WIDTH, ROWS)
    instructions()
