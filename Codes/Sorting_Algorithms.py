...
def algochooser(numbers, paint, label_comparison, something, TYPE_OF_DRAW, speed):
    ...
    if something == "Gnome Sort":
        label_comparison.configure(text="No. of comparisons: 0")
        gnomesort(numbers, paint, label_comparison, speed)
        if TYPE == 0:
            paint(["lawn green"] * len(numbers))
        cmp = 0

    elif something == "Comb Sort":
        label_comparison.configure(text="No. of comparisons: 0")
        combsort(numbers, paint, label_comparison, speed)
        if TYPE == 0:
            paint(["lawn green"] * len(numbers))
        cmp = 0

    elif something == "Cocktail Shaker Sort":
        label_comparison.configure(text="No. of comparisons: 0")
        cocktailshakersort(numbers, paint, label_comparison, speed)
        if TYPE == 0:
            paint(["lawn green"] * len(numbers))
        cmp = 0
...
def gnomesort(number, paint, label_comparison, speed):
    global cmp
    i = 0
    colors = []
    while i < len(number):
        if(i == 0 or number[i] >= number[i-1]):
            ...
            i -= 1
            if TYPE == 0:
                colors = ["#cc0000" if x == number[i] or x == number[i-1] else "antique white" for x in number]
            else:
                colors = [((int)(x * 360) / 950) for x in number]
            paint(colors)
        cmp += 1
...
def combsort(number, paint, label_comparison, speed):
    global cmp
    colors = []
    gap = len(number)
    swaps = True
    while gap > 1 or swaps:
        ...
        for i in range(len(number) - gap):
            ...
            cmp += 1
            if TYPE == 0:
                colors = ["#cc0000" if x == number[j] or x == number[i] else "antique white" for x in number]
            else:
                colors = [((int)(x * 360) / 950) for x in number]
            paint(colors)
            label_comparison.configure(text="No. of comparisons: " + str(cmp))

def cocktailshakersort(number, paint, label_comparison, speed):
    global cmp
    colors = []
