...
class Searching:
    def __init__(self, root, AlgoNameVar):
        ...
        self.frame1 = Frame(root, width=self.FRAME1_X, height=self.FRAME1_Y, bg="#7fe7dc")
        ...
        self.information = {'Linear Search': "Worst Case:O(n)\nAverage Case:O(n/2)\nBest Case:O(1)",
                            ...
                            'Interpolation Search':"bbb",
                            'Jump Search':"ccc"}

        # Algorithm Names
        self.algorithm = ['Linear Search', 'Binary Search','Interpolation Search','Jump Search']
        ...
        self.algo_menu.config(font="calibri", bg="#ced7d8", activebackground="white", cursor="circle")
        ...
        self.frame_btn1 = Frame(self.frame1, width=230, height=40, bg="#7fe7dc")
        self.frame_btn1.pack_propagate(0)
        self.frame_btn1.place(relx=0.0, rely=0.14)
        # Button for generating new array
        self.btn_new = Button(self.frame_btn1, text="Generate", padx=13, pady=3, command=self.new_list, bg="#316879",
        ...
        self.btn_shuffle = Button(self.frame_btn1, text="Shuffle", padx=13, pady=3, command=self.shuffle_list,
                                  bg="#316879", fg="azure", cursor="hand2")
        self.btn_shuffle.place(relx=0.60, rely=0)

        self.label_comparison = Label(self.frame1, text="Enter the element to be searched", bg="#7fe7dc", fg="midnight blue",
                                      font=("Lucida Sans", 9))
        self.label_comparison.place(relx=0.1, rely=0.22)


        # text box for search entry
        self.search_element = Entry(self.frame1, width=24, bd=3, bg='#ced7d8')
        self.search_element.insert(END, '')
        self.search_element.pack_propagate(0)
        self.search_element.place(relx=0.22, rely=0.25)

        # Creating a frame for a new button
        self.frame_btn2 = Frame(self.frame1, width=230, height=40, bg="#7fe7dc")
        self.frame_btn2.grid_propagate(0)
        self.frame_btn2.place(relx=0.0, rely=0.29)
        # Creating a sort button
        self.btn_sort = Button(self.frame_btn2, text="Search", padx=13, pady=3, command=self.search_list,bg="#316879", fg="azure", cursor="hand2")
        ...
        self.scale_size = Scale(self.frame1, label="Size :", orient=HORIZONTAL, from_=10, to=200, length=230,
                                bg="#ced7d8", troughcolor="#ECECEC", variable=self.size_var,
        ...
        self.scale_speed = Scale(self.frame1, label="Speed :", orient=HORIZONTAL, from_=1, to=500, length=230,
                                 bg="#ced7d8", troughcolor="#ECECEC", variable=self.speed_var,
        ...
        self.label_comparison = Label(self.frame1, text="No. of comparisons: 0", bg="#7fe7dc", fg="midnight blue",
                                      font=("Lucida Sans", 12))
        self.label_comparison.place(relx=0.1, rely=0.65)

        # Label for showing the searched element index
        self.label_index = Label(self.frame1, text="", bg="#7fe7dc", fg="VioletRed4",
                                 font=("Lucida Sans", 12))
        self.label_index.place(relx=0.2, rely=0.58)

        # Frame for algorithm info
        self.frame_algo_info = Frame(self.frame1, bg="#D2FFFC", width=230, height=150, relief="sunken", bd=4)
        self.frame_algo_info.grid_propagate(0)
        self.frame_algo_info.place(relx=0.03, rely=0.7)
        # Label for algorithm info
        self.label_avg = Label(self.frame_algo_info, bg="#D2FFFC", fg="midnight blue",
                               text=self.information[self.algo_var.get()], font=("Lucida Sans", 13, "bold"))
        ...
        self.canva = Canvas(self.frame2, width=self.CANVAS_X, height=self.CANVAS_Y, bg="white")
        self.canva.pack()
        ...
        for num in self.numbers:
            self.canva.create_rectangle(self.starting_point, self.CANVAS_Y - num, self.starting_point + self.rec_width,
                                        self.CANVAS_Y, fill="#ced7d8")
            self.label_value = Label(self.canva, text=num, bg="#ced7d8", fg="midnight blue", font=("Lucida Sans", 12))
    ...
    def paint(self, colortype):
        ...
        for i in range(len(self.numbers)):
            ...
            self.label_value = Label(self.canva, text=self.numbers[i], bg="#ced7d8",
                                     fg="midnight blue",
                                     font=("Lucida Sans", 12))
    ...
    def new_list(self):
        ...
        colortype = ["#ced7d8" for x in self.numbers]
